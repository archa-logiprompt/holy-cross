...
class Staff_model extends CI_Model
{
    ...
    public function searchstaffattendancebymonthandyear($staff, $month, $year, $start_date, $get_role)
    {
        $update = $this->db->select('leave_type')->where('staff_id', $staff)->where('date', $start_date)->get('leave_updates')->row()->leave_type;
        if ($update) {
            $result['type'] = $update;
            $result['updated'] = "updated";
        } else {
            $id = (implode('', explode('N', $staff)));
            $result['in'] = $this->db->select('*')->where('DeviceId', 1)->where('C4', 0)->where('UserId', $id)->where('DATE(LogDate)', $date)->get('DeviceLogs_' . $month . '_' . $year)->row();
            echo $this->db->last_query();
            exit;
            var_dump($result);
            exit;

            $result['out'] = $this->db->select('*')->where('DeviceId', 1)->where('C4', 1)->where('UserId', $id)->where('DATE(LogDate)', $date)->get('DeviceLogs_' . $month . '_' . $year)->row();
            ...
            if ((int)$date_split[0] >= 2024 && (int)$date_split[1] >= 10) {
                if ($get_role->staff_type == '0') {
                    if ($interval->h >= 8) {
                        $result['type'] = "P";
                    } else {
                        $result['type'] = "A";
                    }
                } elseif ($get_role->staff_type == '1') {
    ...
    {
        $result = $this->db->select('leave_updates.*,staff.name,staff.surname')->join('staff', 'staff.id=leave_updates.staff_id')->get('leave_updates')->result_array();
        return $result;
    }
}
