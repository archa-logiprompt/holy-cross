...
function getperiodtime($period_id, $period)
{
	$CI = &get_instance();
	$CI->load->database();


	$time = $CI->db->select($period . "_from," . $period . "_to")->where('id', $period_id)->get('period_timing')->row_array();
	$time = $time[$period . "_from"] . " - " . $time[$period . "_to"];
